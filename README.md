...
The [The Reactive Manifesto](https://www.reactivemanifesto.org/) encourages the systems to communicate through messaging. Even though our modules are in close promixity to each other, we wouldbe using messaging to pass messages and receive messages between them. Messages will also be used to create reports and audit trail.

Using messages will make it easier to later separate these modules when the application evolves to microservices. The in-memory message bus will be replaced my a message queue.

**9. Continuous Monitoring**

According to [The 12 Factor Apps](https://12factor.net/) the logs generated by the App can be routed to a file, and also streamed to an analysis system so that it provides the following benifits:

1. Introspecting an app’s behavior over time.
...
Each module will log errors to its own file. Which analysis engine to be used will be decided later. (Probably - ELK in combination with Spark)
